...
class LoggerMoudle {
    ...
    logAPI(api, method, uid, succ, consumeTime, reqID, param, response, support_plts) {
        this.keeper.forEach(v => {
            // 考虑某些日志可能只需要记录到特定的平台
            if (support_plts && support_plts.indexOf(v.name) < 0)
                return;
    ...
    apiBegin(URI, method, uid, param, reqID, timeOut) {
        ...
        // 这里设置一个超时时间
        let t = setTimeout(this.apiEnd.bind(this, reqID), timeOut || this.apiTimeout);
        let s = {
            URI: URI,
            method: method,
            uid: (uid || "0").toString(),
